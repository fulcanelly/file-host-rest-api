...
async function generateAccessToken(id, sessionId) {
  const accessToken = jwt.sign({
    ...
    id,
    sessionId
  }, ACCESS_TOKEN_SECRET_KEY, { algorithm: 'HS256', expiresIn: config.accessTokenExpiresIn })
...
async function generateRefreshToken(id, sessionId) {
  ...
  const refreshToken = jwt.sign({
    ...
    id,
    sessionId
  }, REFRESH_TOKEN_SECRET_KEY, { algorithm: 'HS256', expiresIn: config.refreshTokenExpiresIn })
